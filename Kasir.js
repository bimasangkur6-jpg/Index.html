...
// ================= TUNING GRAFIK CHART.JS AGAR MENDUKUNG DARK MODE =================
...
// ================= BRANKAS DATA SECURITY (SAMPEL SAFETY UX) =================
...
function imporDataDariFile(event) {
    ...
    // VALIDASI UX SAFETY: Menghindari ketidaksengajaan klik file rusak/salah
    if (!confirm("⚠️ PERINGATAN STRATEGIS: Memuat file ini akan menghapus dan menimpa seluruh data dashboard Anda saat ini. Apakah Anda yakin ingin melanjutkan?")) {
        event.target.value = ''; 
        return;
    }
    ...
    reader.onload = function (e) {
        try {
            ...
            if (parsed.kasAwal !== undefined && parsed.produk && parsed.transaksi) {
                ...
                SinkronisasiFormKasAwal();
                renderSeluruhSistem();
                alert("🎉 Sukses! Seluruh master data dan model keuangan berhasil dipulihkan.");
            } else {
                alert("Format file JSON tidak sesuai dengan standar sistem!");
            }
...
function hapusMasterProduk(id, event) {
    if (event) event.stopPropagation(); // Mencegah terpicunya transaksi kasir
    if (confirm("Hapus master produk ini secara permanen dari etalase?")) {
...
// FITUR BARU: RESTOCK LOGISTIK CEPAT TANPA HAPUS MASTER BARANG
function restockProdukCepat(id, event) {
    if (event) event.stopPropagation(); // Stop bubbling ke klik kartu belanja
    const prod = databaseUang.produk.find(p => p.id === id);
    if (!prod) return;

    let tambahan = prompt(`Tambah Stok Gudang untuk "${prod.nama}" (Stok Saat Ini: ${prod.stok} Unit):`, "10");
    if (tambahan === null) return; // Batalkan jika user menekan cancel

    let jumlah = parseInt(tambahan);
    if (isNaN(jumlah) || jumlah < 0) {
        alert("Masukkan angka jumlah restock yang valid!");
        return;
    }
...
function tambahKeKeranjang(idProduk) {
    ...
    if (!prod || prod.stok <= 0) return alert("Stok barang di gudang sudah habis!");

    const item = keranjangKasir.find(k => k.idProduk === idProduk);
    if (item) {
        if (item.qty >= prod.stok) return alert("Pembelian tidak boleh melebihi sisa stok gudang!");
        item.qty++;
    } else {
    ...
    keranjangKasir.forEach(item => {
        ...
        tr.innerHTML = `
            <td>
                ...
                <div style="font-size: 10px; color:#9ca3af; margin-top:4px;">Gudang Sisa: ${sisaStokRealTime} unit</div>
            ...
            <td style="font-weight:bold; color:var(--text-primary);">${formatMataUang(subtotal)}</td>
...
function prosesSelesaiBayar() {
    if (keranjangKasir.length === 0) return alert("Keranjang belanja masih kosong!");
    const total = parseFloat(document.getElementById('cart-total-bill').innerText.replace(/[^0-9]/g, '')) || 0;
    const bayar = ambilAngkaMentah('pay-amount');

    if (bayar < total) return alert("Uang tunai yang diterima belum mencukupi!");
...
function cetakStrukThermal() {
    if (!logStrukTerakhir) return alert("Belum ada transaksi log histori!");
    buatStrukturHtmlStruk();
    window.print();
}
...
function renderSeluruhSistem() {
    ...
    databaseUang.produk.forEach(prod => {
        ...
        // MODIFIKASI: Menambahkan tombol aksi manajemen stok mini di dalam etalase jualan
        card.innerHTML = `
            ...
            <div class="prod-info">
                ...
                <div class="prod-meta-actions">
                    <button class="btn-restock" onclick="restockProdukCepat(${prod.id}, event)">➕ Stok</button>
                    <button class="btn btn-danger" style="padding: 2px 6px; font-size: 0.7rem; border-color:#ef4444;" onclick="hapusMasterProduk(${prod.id}, event)">🗑️</button>
                </div>
            </div>
        `;
